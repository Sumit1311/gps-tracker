...
void setup() {
  ...
  String responses[10];
  /*for(int i=0;i<10;i++){
    memset(responses[i], '\0', GPS_STRING_SIZE);
  }*/
  sendATcommand("AT+CREG?",responses , 10000);
...
void loop() {
  /*getGPSCoordinates();
    if (sendATcommand("AT+CFUN=1", "OK", 500)) {
    if (sendATcommand2("AT+CIPMUX=0", "OK", "ERROR", 1000) == 1)
    {
      ...
      if (sendATcommand("AT+CGDCONT=1,\"IP\",\"www\"", "OK", 10000) == 1) {
        if (sendATcommand("AT+CGACT=1,1", "OK", 50000) == 1) {
          if (sendATcommand("AT+CGATT=1", "OK", 1000) == 1) {
            if (sendATcommand2("AT+CSTT", "OK",  "ERROR", 10000) == 1)
            { // Waits for status IP START
              while (sendATcommand("AT+CIPSTATUS", "START", 500)  == 0 );
              // Brings Up Wireless Connection
              if (sendATcommand2("AT+CIICR", "OK", "ERROR", 50000) == 1)
              { // Waits for status IP GPRSACT
                ...
                if (sendATcommand2("AT+CIFSR", ".", "ERROR", 10000) == 1)
                {
                  ...
                  // Opens a TCP socket
                  if (sendATcommand2(aux_str, "CONNECT OK", "CONNECT FAIL", 50000) == 1)
                  {
    ...
    Serial.println("Error setting CFUN");
    }*/
}
...
int8_t sendATcommand(char* ATcommand, String *answer, unsigned int timeout) {
  uint8_t x = 0, isAnswered = 0;
  ...
  x = 0;
  previous = millis();
  int numberOfCharacter, count = 0;
  //Serial.println("First Char : ");
  //Serial.println(readChar);

  do {
    if (myGSM.available() != 0) {
      numberOfCharacter = myGSM.readBytesUntil('\n', response, GPS_STRING_SIZE);
      String stringOne = String(response);   
      Serial.println("Response1 :");
      Serial.println(stringOne.c_str());
      answer[count]=stringOne;
      count++;
      memset(response, '\0', GPS_STRING_SIZE);
    }
  } while ((numberOfCharacter != 0) && ((millis() - previous) < timeout));
